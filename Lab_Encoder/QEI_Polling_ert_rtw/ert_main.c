...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) QEI_Polling_M->Timing.taskTime0;
  QEI_Polling_step();
...
int main(int argc, char **argv)
{
  ...
  MX_TIM4_Init();
  MX_TIM8_Init();
  rtmSetErrorStatus(QEI_Polling_M, 0);
  ...
  QEI_Polling_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(QEI_Polling_M->extModeInfo, &rtmGetTFinal
    (QEI_Polling_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(QEI_Polling_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(QEI_Polling_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
